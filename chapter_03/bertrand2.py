...
#  file:  bertrand2.py
#
#  Random interior point as midpoint of chord
#
#  RTK, 23-Apr-2022
...
if (len(sys.argv) == 1):
    print()
    print("bertrand2 <npts> <output> pcg64|mt19937|minstd|urandom|rdrand|<file> [<seed>]")
    print()
...
Y = []
for i in range(N):
    a = -r + 2*r*rng.random()               #  random point inside the circle
    b = -r + 2*r*rng.random()
    while (a**2 + b**2) > r**2:
        a = -r + 2*r*rng.random()
        b = -r + 2*r*rng.random()
