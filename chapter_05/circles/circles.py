...
#  file:  circles.py
#
#  Optimize placing points in the unit square, which is
#  equivalent to packing circles of the same radius.
#
#  RTK, 25-May-2022
...
def main():
    ...
    xy = p.reshape((n,2))
    for i in range(n):
        plt.plot(xy[i,0], xy[i,1], marker='o', linestyle='none', color='k')
    plt.xlim((0,1))
    plt.ylim((0,1))
    plt.xlabel("$x$")
    plt.ylabel("$y$")
    ...
    plt.savefig(outdir+"/plot.png", dpi=300)
